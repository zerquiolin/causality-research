...
def get_scm_stats(scm):
    ...
    return {
        "num_nodes": num_nodes,
        "num_edges": num_edges,
        "num_roots": len([v for v, d in in_degrees.items() if d == 0]) if in_degrees is not None else None,
        "num_leaves": len([v for v, d in out_degrees.items() if d == 0]) if out_degrees is not None else None,
