...
from causalitygame.generators.outcome.base import (
...
from causalitygame.scm.nodes.db import (
    DatabaseDefinedCategoricSCMNode,
    DatabaseDefinedNumericSCMNode,
)
from causalitygame.scm.nodes.computed import (
...
class DatabaseSCM(SCM):
    def __init__(
        ...
        self.covariates = [
            c
            for c in factual_df.columns
            if c not in self.intervention_variables and c not in self._outcome_vars
        ]
        self.outcome_generators = outcome_generators
        self.overwrite_factual_outcomes = overwrite_factual_outcomes
        self.only_factual_outcomes_in_interventionless_sampling = (
            only_factual_outcomes_in_interventionless_sampling
        )
        self.allow_duplicate_interventions_per_covariate_combination = (
            allow_duplicate_interventions_per_covariate_combination
        )
        ...
        # save the dataframe
        self.df = factual_df[
            self.covariates + self.intervention_variables + self._outcome_vars
        ].copy()
        ...
        for outcome_var, generator in self.outcome_generators.items():
            gen = (
                generator
                if self.overwrite_factual_outcomes
                else ComplementaryOutcomeGenerator(generator)
            )
            gen.fit(
        ...
        for name in self.var_names:
            if (
                self.df[name].dtype in [float, int, np.number]
                and len(pd.unique(self.df[name])) > 10
            ):
                if name in self._outcome_vars:
                    ...
                    node = DatabaseDefinedNumericSCMNode(
                        ...
                        accessibility=(
                            ACCESSIBILITY_CONTROLLABLE
                            if name in self.intervention_variables
                            else ACCESSIBILITY_OBSERVABLE
                        ),
                        random_state=random_state,
                    )
            else:
    ...
    def generate_samples(
        ...
        for node_name, node in [
            ...
            if node_name in interventions:
                ...
                if isinstance(
                    node,
                    (DatabaseDefinedCategoricSCMNode, DatabaseDefinedNumericSCMNode),
                ):
                    ...
                    sample_for_col = node.generate_values(
                        parent_values=sample, random_state=rs
                    )
            ...
            t_start_update = time()
            compatibilities_of_generated_vals_in_column = (
                self.df[node_name].values[None, :] == sample_for_col[:, None]
            )
            ...
            t += added_runtime
            self.logger.info(
                f"Node {node_name} added a runtime of {added_runtime}, total elapsed time for generation: {t}. Updating possabilities took {t_end - t_start_update}"
            )
    ...
    def to_dict(self) -> Dict:
        ...
        return {
            ...
            "outcome_generators": {
                k: ocg.to_dict() for k, ocg in self.outcome_generators.items()
            },
    ...
    def from_dict(cls, data: Dict) -> "SCM":
        ...
        """
        data = data.copy()
        data["factual_df"] = pd.read_json(
            StringIO(data["factual_df"]), orient="records", precise_float=True
        )
        ...
        data["outcome_generators"] = {
            k: OutcomeGenerator.from_dict(v)
            for k, v in data["outcome_generators"].items()
        }
        return cls(**data)
