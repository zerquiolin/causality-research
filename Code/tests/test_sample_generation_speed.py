...
base_path = Path(f"{Path(__file__).parent.parent}/causalitygame/data/scm/")
...
def test_physical_problem_sample_generation_speed(filename):
    ...
    assert t_end - t_start < 1, f"Runtime for {n_samples} must be less than a second but was {t_end - t_start}s"
